...
class Config {
 public:
  ...
  const ServerConf *getServConfig(const int port, const std::string &host);
  const LocationConf &getLocationConfig(const ServerConf *serv_conf, const std::string &path) const;
